...
RippleButton {
    ...
    // Task dot — uses primary color (M3 already)
    Rectangle {
        width: 8; height: 8
        radius: Appearance.rounding.full
        color: (taskList.length > 0 && isToday !== -1 && !bold)
        ? toggled ? Appearance.m3colors.m3onPrimary : Appearance.colors.colPrimary
        : "transparent"
    ...
    LazyLoader {
        ...
        Behavior on itemScale   { animation: Appearance.animation.elementMoveFast.numberAnimation.createObject(this) }
        Behavior on itemOpacity { animation: Appearance.animation.elementMoveFast.numberAnimation.createObject(this) }
    ...
    MouseArea {
        anchors.fill: parent
        hoverEnabled: true
        onEntered: { if (button.taskList.length > 0 && button.isToday !== -1 && !button.bold) button.showPopup = true }
    ...
    StyledText {
        ...
        // M3: already used m3onPrimary — keep; use m3onSurface for normal days
