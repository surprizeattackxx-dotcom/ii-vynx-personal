...
Singleton {
    ...
    function thumbnailSizeNameForDimensions(width: int, height: int): string {
        // When called from ThumbnailImage with sourceSize before load, width/height are 0.
        // Default to "large" (256px) which is the best general-purpose size.
        if (width <= 0 || height <= 0) return "large";
